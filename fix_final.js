...
const analyticsSpec = `const { test, expect } = require('@playwright/test');
...
test.describe('Advanced Analytics Module', () => {
  ...
  test('should display missing data warning when neck is not provided', async ({ page }) => {
    ...
    await expect(page.locator('#analytics-dashboard').getByText('Brak danych do wyliczenia BF%')).toBeVisible();
    await expect(page.locator('#analytics-dashboard').getByText('Uzupełnij: Szyja')).toBeVisible();
  });
});
`;

fs.writeFileSync('tests/e2e/analytics.spec.js', analyticsSpec);
console.log("Analytics completely rewritten!");

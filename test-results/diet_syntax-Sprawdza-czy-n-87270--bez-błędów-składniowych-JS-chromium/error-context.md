...
Locator: locator('#app-header-title')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#app-header-title')
...
```ts
  ...
  11 |   await page.goto('file://' + require('path').resolve(__dirname, '../../index.html'));
  12 | 
  13 |   // Sprawdzamy czy okno profilu lub napis Uki Bodybuild jest widoczny by potwierdzić że skrypt w ogóle ruszył
> 14 |   await expect(page.locator('#app-header-title')).toBeVisible();
     |                                                   ^ Error: expect(locator).toBeVisible() failed
  15 | 
  16 |   // Oczekujemy, że nie było żadnych błędów JS
